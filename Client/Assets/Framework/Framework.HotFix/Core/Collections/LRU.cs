...
#if UNITY_EDITOR
        /// <summary>
        /// [仅编辑器] 按 key 返回缓存内容快照，供 UI Inspector 调试窗口池使用。
        /// </summary>
        public List<KeyValuePair<TKey, TValue>> DebugGetKeyValuePairs()
        {
            var pairs = new List<KeyValuePair<TKey, TValue>>(entries.Count);
            foreach (var pair in entries)
            {
                pairs.Add(new KeyValuePair<TKey, TValue>(pair.Key, pair.Value.Value));
            }
