...
namespace NFramework.ModuleSystem
{
    ...
    public struct InstanceIdStruct
    {
        ...
        {
            ulong result = (ulong)id;
            this.Value = (uint)(result & IDGeneratorSystem.Mask18bit);
            result >>= 18;
            this.Time = (uint)result;
        }
    ...
    public class IDGeneratorSystem : FrameworkSystemModuleBase
    {
        ...
        public override void Awake()
        {
            ...
            {
                NFROOT.I.GetSystem<LoggerSystem>().Warning
                    ?.Print($"lastInstanceIdTime less than 0: {this.lastInstanceIdTime}");
            ...
            {
                NFROOT.I.GetSystem<LoggerSystem>().Warning?.Print($"lastIdTime less than 0: {this.lastIdTime}");
                this.lastIdTime = 1;
            }
            ...
            {
                NFROOT.I.GetSystem<LoggerSystem>().Warning?.Print($"lastUnitIdTime less than 0: {this.lastUnitIdTime}");
        ...
        {
            uint a = (uint)((GetSystem<TimeInfoSystem>().ClientNow() - this.epoch2020) / 1000);
            return a;
        }

        private uint TimeSinceThisYear()
        {
            uint a = (uint)((GetSystem<TimeInfoSystem>().ClientNow() - this.epochThisYear) / 1000);
            return a;
        }
        ...
        public long GenerateInstanceId()
        {
            ...
            else
            {
                ...
                if (this.instanceIdValue > IDGeneratorSystem.Mask18bit - 1) // 18bit
                {
                    ++this.lastInstanceIdTime; // 借用下一秒
                    this.instanceIdValue = 0;

                    NFROOT.I.GetSystem<LoggerSystem>().Error
                        ?.Print($"instanceid count per sec overflow: {time} {this.lastInstanceIdTime}");
                }
            }
        ...
        public long GenerateId()
        {
            ...
            else
            {
                ...
                if (value > ushort.MaxValue - 1)
                {
                    ...
                    NFROOT.I.GetSystem<LoggerSystem>().Error
                        ?.Print($"id count per sec overflow: {time} {this.lastIdTime}");
                }
            }
        ...
        public long GenerateUnitId(int zone)
        {
            ...
            else
            {
                ...
                if (this.unitIdValue > ushort.MaxValue - 1)
                {
                    ...
                    NFROOT.I.GetSystem<LoggerSystem>().Error
                        ?.Print($"unitid count per sec overflow: {time} {this.lastUnitIdTime}");
                }
            }
