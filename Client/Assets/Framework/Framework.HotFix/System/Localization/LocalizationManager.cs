...
namespace NFramework.ModuleSystem.Module.LocalizationModule
{
    ...
    /// 本地化管理器
    /// </summary>
    public class LocalizationM : FrameworkSystemModuleBase
    {
        ...
        public void SetLanguageData(Func<int, string, string> getTextFunc)
        {
            ...
            _isLoaded = true;
            NFROOT.I.GetSystem<LoggerSystem>().Log?.Print($"本地化数据设置成功，支持语言: {string.Join(", ", _languageKeys)}");
        }
        ...
        public string GetText(int id)
        {
            ...
            {
                this.Warning?.Print($"本地化文本未找到: {id}");
                return $"[{id}]";
            }
