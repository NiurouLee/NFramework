...
namespace NFramework.ModuleSystem
{
    public partial class UISystem
    {
        ...
        private void _Close(Window inWindow)
        {
            ...
            else
            {
                this.GetSystem<LoggerSystem>()
                    ?.ErrStack(
                        $"UIM::Close window not in WindowRequestDictionary, WindowName:{inWindow.GetType().Name}");
            }
        }
        ...
        private async void StartCloseAsync(WindowRequest inWindowRequest)
        {
            if (inWindowRequest == null)
            {
                return;
            ...
            if (this.HaveCloseAnim(inWindowRequest, out float time))
            {
                ...
                this.CloseWithAnimation(inWindowRequest);
            }
            else
            {
                this._OnAnimClose(inWindowRequest);
            }
        }

        private void CloseWithAnimation(WindowRequest inWindowRequest)
        {
            inWindowRequest.SetStage(WindowRequestStage.WindowCloseAnim);
            this.PlayCloseAnim(inWindowRequest.Config, inWindowRequest);
            if (inWindowRequest.IsCanceled)
            {
                return;
            }
            
            _OnAnimClose(inWindowRequest);
        }
