...
public class EasyTouch : MonoBehaviour {
	...
	[NonSerialized]
	public bool temporaryDisabled = false;
	[NonSerialized]
	public Func<EventName, Gesture, bool> eventInterceptor = null;
	#endregion
    ...
    void InitEasyTouch(){
		...
		if (secondFingerTexture==null && enableSimulation){
				secondFingerTexture = AssetDatabase.LoadAssetAtPath<Texture>("Assets/Scripts/Thirdparty/EasyTouch/Res/secondFinger.png");
			}
		#endif	
	}
	...
	void Update(){
		...
		if (!temporaryDisabled && enable && EasyTouch.instance==this)
        {
			int i;
			// How many finger do we have ?
			int count = input.TouchCount();
            if (oldTouchCount == 2 && count <= 0) 
            {
                oldTouchCount = 0;
                CreateGesture2Finger(EventName.On_TouchUp2Fingers, Vector2.zero, Vector2.zero, Vector2.zero, 0, SwipeDirection.None, 0, Vector2.zero, 0, 0, 0);
                return;
            }
	...
	private void OneFinger(int fingerIndex){
		...
		if (singleDoubleTap[fingerIndex].inDoubleTap) singleDoubleTap[fingerIndex].time += deltaTime;
	...
	private void TwoFinger(){
		...
		if ( twoFinger.currentGesture==GestureType.None)
        {
            ...
            //Debug.LogError(allowUIDetection.ToString() + "-----" + GetTwoFingerPickedUIElement().ToString());
			// UnityGUI
			if (allowUIDetection){
		...
		if (singleDoubleTap[99].inDoubleTap) singleDoubleTap[99].time += deltaTime;
	...
	private bool GetTwoFingerPickedUIElement(){
        ...
        if (AreBothTouchesInNotchArea()) return true;
		...
		if (twoFingerPickMethod == TwoFingerPickMethod.Finger){
			if (IsScreenPositionOverUI( fingers[twoFinger.finger0].position )){
				...
				if (IsScreenPositionOverUI( fingers[twoFinger.finger1].position )){
					...
					if (tmp2 == tmp || tmp2.transform.IsChildOf( tmp.transform) || tmp.transform.IsChildOf( tmp2.transform) || IsInSameCanvas(tmp.transform, tmp2.transform))
                    {
    ...
    bool AreBothTouchesInNotchArea()
    {
        if (Input.touchCount < 2)
            return false;

        Vector2 pos1 = fingers[twoFinger.finger0].position;
        Vector2 pos2 = fingers[twoFinger.finger1].position;

        float notchYThreshold = Screen.safeArea.yMax;

        return pos1.y > notchYThreshold || pos2.y > notchYThreshold;
    }

    bool IsTouchInSafeArea(Vector2 touchPos)
    {
        Rect safeArea = Screen.safeArea;
        return safeArea.Contains(touchPos);
    }

    Canvas GetParentCanvas(Transform node)
    {
        return node.GetComponentInParent<Canvas>();
    }

    bool IsInSameCanvas(Transform nodeA, Transform nodeB)
    {
        return GetParentCanvas(nodeA) == GetParentCanvas(nodeB);
    }
    ...
    public void RaiseEvent(EventName evnt, Gesture gesture){

		if (eventInterceptor != null && !eventInterceptor (evnt, gesture))
		{
			return;
		}
			...
			while (!returnValue && i<nGUICameras.Count){
				...
				if (useUnity2D)
				{
					...
					if (m2DPlane.Raycast(ray, out dist))
					{
						...
						returnValue = Physics2D.OverlapPoint(hitPos, mask);
					}
					else
					{
						returnValue = false;
				...
				{
					returnValue = Physics.Raycast( ray, out hit, nGUICameras[i].farClipPlane - nGUICameras[i].nearClipPlane, mask );
				}
				i++;
			}
	...
	{
		On_Cancel = null;
		On_Cancel2Fingers = null;
		On_TouchStart = null;
		On_TouchDown = null;
		On_TouchUp = null;
		On_SimpleTap = null;
		On_DoubleTap = null;
		On_LongTapStart = null;
		On_LongTap = null;
		On_LongTapEnd = null;
		On_DragStart = null;
		On_Drag = null;
		...
		On_Swipe = null;
		On_SwipeEnd = null;
		On_TouchStart2Fingers = null;
		On_TouchDown2Fingers = null;
		On_TouchUp2Fingers = null;
		On_SimpleTap2Fingers = null;
		On_DoubleTap2Fingers = null;
		On_LongTapStart2Fingers = null;
		On_LongTap2Fingers = null;
		On_LongTapEnd2Fingers = null;
		On_Twist = null;
		On_TwistEnd = null;
		On_Pinch = null;
		On_PinchIn = null;
		On_PinchOut = null;
		On_PinchEnd = null;
		On_DragStart2Fingers = null;
		On_Drag2Fingers = null;
		On_DragEnd2Fingers = null;
		On_SwipeStart2Fingers = null;
		On_Swipe2Fingers = null;
		On_SwipeEnd2Fingers = null;
		On_EasyTouchIsReady = null;
		On_OverUIElement = null;
		On_UIElementTouchUp = null;
		instance = null;
		m2DPlane = new Plane(Vector3.back, 0f);
	}
