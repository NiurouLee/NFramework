...
namespace UnityZedIntegration
{
    public class Zed
    {
        ...
        {
            // 不要用 IsSupportedFile 拦截打开：VS 生成器对 PackageCache、部分包路径等会返回 false，
            // 但 Zed 仍可正常编辑；若在此处 return，Unity 表现为双击脚本无任何反应。
            if (!string.IsNullOrEmpty(filePath) && !generator.IsSupportedFile(filePath))
                Debug.LogWarning($"[ZED] File '{filePath}' is not in generated project; opening in Zed anyway.");
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                if (line >= 0)
                {
                    // Windows 绝对路径含 "C:"，与 Zed 的 path:line:column 语法叠在一起易解析歧义，改用正斜杠更稳妥。
#if UNITY_EDITOR_WIN
