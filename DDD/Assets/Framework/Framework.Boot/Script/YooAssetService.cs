    ...
    public class YooAssetService
    {
        ...
        public async UniTask<bool> InitializeAndUpdate()
        {
            try
            {
                ...
                FinishUpdate();
                AOTLog.Info("YooAsset初始化和更新流程全部完成");
                return true;
            }
            catch (Exception e)
            {
                AOTLog.Info($"YooAsset更新失败: {e.Message}");
        ...
        private async UniTask<bool> InitializePackage()
        {
            ...
            AOTLog.Info($"当前运行模式: {currentPlayMode}");
            // 创建资源包裹类
            ...
            if (_package == null)
            {
                ...
                AOTLog.Info($"创建新的资源包: {_packageName}");
            }
            else
            {
                AOTLog.Info($"使用已存在的资源包: {_packageName}");
            }
            ...
            else if (currentPlayMode == EPlayMode.WebPlayMode)
            {
                ...
                this.Log?.Print($"远程资源地址: {platformURL}");
                IRemoteServices remoteServices = new RemoteServices(platformURL, platformURL);
     ...
     WechatFileSystemCreater.CreateFileSystemParameters(packageRoot, remoteServices);
            ...
            {
                AOTLog.Info($"初始化资源包失败: {initializationOperation.Error}");
        ...
        private async UniTask<bool> RequestPackageVersion()
        {
            ...
            {
                AOTLog.Info($"请求资源版本失败: {operation.Error}");
                OnError?.Invoke("请求资源版本失败!");
                return false;
            }

            AOTLog.Info($"Request package version: {operation.PackageVersion}");
        ...
        private async UniTask<bool> UpdatePackageManifest()
        {
            ...
            if (operation.Status != EOperationStatus.Succeed)
            {
                AOTLog.Info($"更新资源清单失败: {operation.Error}");
        ...
        private async UniTask<bool> CheckAndDownloadFiles()
        {
            ...
            {
                AOTLog.Info("No files to download!");
                return true;
            }
            ...
            AOTLog.Info($"发现需要下载的文件: {totalDownloadCount}个, 总大小: {totalDownloadBytes}字节");
        ...
        private async UniTask<bool> DownloadFiles()
        {
            ...
            {
                AOTLog.Info($"下载文件失败: {errorData.FileName}, {errorData.ErrorInfo}");
