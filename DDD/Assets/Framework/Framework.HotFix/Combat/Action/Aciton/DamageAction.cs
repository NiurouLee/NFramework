...
namespace NFramework.ModuleSystem.Combat
{
    ...
    public class DamageAction : Entity, IActionExecution
    {
        ...
        private void PreProcess()
        {
            ...
            {
                //暴击概率
                isCritical = (RandomUtil.RandomRate() / 100f) <
                             Creator.GetComponent<AttributeComponent>().CriticalProbability.Value;
                //攻击方攻击力
                DamageValue = (int)Creator.GetComponent<AttributeComponent>().Attack.Value;
                //受击方防御力
                DamageValue = Mathf.CeilToInt(Mathf.Max(1,
                    DamageValue - Target.GetComponent<AttributeComponent>().Defense.Value));
                if (isCritical)
                {
            ...
            if (this.DamageSource == DamageSource.Skill)
            {
                ...
                DamageValue = SourceAssignAction.AbilityEffect.GetComponent<AbilityEffectDamageComponent>()
                    .GetDamageValue();
                DamageValue = Mathf.CeilToInt(Mathf.Max(1,
                    DamageValue - Target.GetComponent<AttributeComponent>().Defense.Value));
                if (isCritical)
            ...
            AbilityEffectDamageReduceWithTargetCountComponent component = SourceAssignAction.AbilityEffect
                .GetComponent<AbilityEffectDamageReduceWithTargetCountComponent>();
            if (component != null)
            {
                var targetCounterComponent =
                    SourceAssignAction.AbilityItem.GetComponent<AbilityItemTargetCounterComponent>();
