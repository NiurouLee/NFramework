...
namespace NFramework.ModuleSystem.Combat
{
    public class StatusComponent : Entity
    {
        ...
        public UnOrderMultiMapList<int, StatusAbility> statusDict = new UnOrderMultiMapList<int, StatusAbility>();

        public StatusAbility AttachStatus(int StatusId)
        {
            StatusConfigObject
                statusConfigObject = null; //= NFROOT.Instance.GetModule<ResM>().Load<StatusConfigObject>(string.Empty);
            ...
            foreach (var item in Combat.Children.Values)
            {
                if (item is StatusAbility status)
                {
                    ...
                    foreach (var effect in status.GetComponent<AbilityEffectComponent>().AbilityEffectList)
                    {
                        ...
                        {
                            tempActionControl = tempActionControl |
                                                actionControlComponent.ActionControlEffect.ActionControlType;
                        }
                    }
                }
            }
