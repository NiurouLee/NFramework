...
namespace NFramework.ModuleSystem.Module.Coroutine
{
    public class CoroutineHandler : CustomYieldInstruction
    {
        ...
        public void Start()
        {
            if (Running)
            {
                NFROOT.Instance.GetSystem<LoggerSystem>()?.Log?.Print("当前协程未完成");
                return;
            }

            if (Coroutine == null)
            {
                NFROOT.Instance.GetSystem<LoggerSystem>()?.Log?.Print("协程未指定");
                return;
            }

            Running = true;
            NFROOT.Instance.StartCoroutineUnity(CallWrapper());
        }
