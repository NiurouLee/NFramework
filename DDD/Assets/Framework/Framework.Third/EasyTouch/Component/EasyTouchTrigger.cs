...
public class EasyTouchTrigger : MonoBehaviour {
	...
	private void SubscribeEasyTouchEvent(){
        ...
        if(IsRecevier4(EasyTouch.EventName.On_Cancel2Fingers))
            EasyTouch.On_Cancel2Fingers += On_DragEnd2Fingers;
    ...
    {
        TriggerScheduler(EasyTouch.EventName.On_Cancel2Fingers, gesture);
    }
	...
	public EasyTouchReceiver AddTrigger(EasyTouch.EventName ev, string triggerName = "New trigger"){
	...
	private void TriggerScheduler(EasyTouch.EventName evnt, Gesture gesture){
		...
		foreach( EasyTouchReceiver receiver in receivers){
			...
			if (receiver.enable && receiver.eventName == evnt){
				if ((receiver.restricted && gesture.pickedObject == gameObject) 
				    || (!receiver.restricted && (receiver.gameObject == null || receiver.gameObject == gesture.pickedObject))){
