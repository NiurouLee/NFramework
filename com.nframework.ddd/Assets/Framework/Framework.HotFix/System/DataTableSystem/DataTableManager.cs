...
namespace NFramework.ModuleSystem.Module.DataTableModule
{
    ...
    public class DataTableM : FrameworkSystemModuleBase
    {
        ...
        public bool LoadTable<T>(string tableName) where T : TableBase, new()
        {
            if (!_dataTableBytesCache.TryGetValue(tableName, out byte[] bytes))
            {
                this.Error?.Print($"数据表字节数据未找到: {tableName}");
                return false;
            }
            ...
            try
            {
                ...
                this.Log?.Print($"数据表加载成功: {typeof(T).Name}");
                return true;
            }
            catch (Exception e)
            {
                this.Error?.Print($"数据表加载失败: {typeof(T).Name}, {e.Message}");
                return false;
            }
        }
        ...
        public async UniTask LoadAllDataTablesByTagAsync(uint priority)
        {
            ...
            TextAsset assets = null;//new TextAsset();//await NFROOT.I.G<ResM>().LoadAsync<TextAsset>("DataTable", priority);

            if (assets.Count == 0)
            {
                this.Error?.Print("未找到任何DataTable文件");
                return;
            }
            ...
            foreach (var kvp in assets)
            {
                ...
                if (textAsset != null)
                {
                    ...
                    this.Log?.Print($"配置表加载成功: {dataTableName} (大小: {textAsset.bytes.Length} 字节)");
                }
            }

            IsLoaded = true;
            this.Log?.Print($"加载DataTable成功共 {assets.Count} 个DataTable文件");
        }
*/
