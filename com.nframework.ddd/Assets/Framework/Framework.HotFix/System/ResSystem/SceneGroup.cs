...
namespace NFramework.ModuleSystem
{
    ...
    /// 负责场景的加载、卸载和切换
    /// </summary>
    public class SceneGroup : NObject
    {
        public void Awake()
        {
        ...
        /// </summary>
        private Dictionary<string, SceneHandle> _loadedScenes;
        ...
        public async UniTask<bool> LoadSceneAsync(string scenePath, LoadSceneMode loadMode, uint priority, Action<float> onProgress = null)
        {
            ...
            {
                this.Error?.Print("[SceneGroup] 场景路径为空");
                return false;
            }
            ...
            {
                this.Error?.Print("[SceneGroup] 加载场景失败: {0}, 错误: {1}", scenePath, handle.LastError);
                return false;
            }
            ...
            this.Log?.Print("[SceneGroup] 场景加载完成: {0}, 模式: {1}", scenePath, loadMode);
            return true;
        }
        ...
        public async UniTask<bool> UnloadSceneAsync(string scenePath)
        {
            ...
            {
                this.Error?.Print("[SceneGroup] 不能直接卸载主场景，请使用 LoadSceneAsync 切换到新场景");
                return false;
            }
            ...
            {
                this.Warning?.Print("[SceneGroup] 场景未加载，无需卸载: {0}", scenePath);
                return true;
            }
            ...
            this.Log?.Print("[SceneGroup] 场景卸载完成: {0}", scenePath);
            return true;
        }
        ...
        public async UniTask UnloadAllAdditiveScenes()
        {
            ...
            this.Log?.Print("[SceneGroup] 已卸载所有叠加场景，共 {0} 个", scenesToUnload.Count);
        }
